...
use App\Models\backend\Featuresmodel;

class Features extends Admincontroller {
    public $_table_names 	= 'features';		//set table
    public $_subView = 'admin/features/';			//set subview
    public $_mainView = 'admin/_layout_main';		//set mainview
    public $_redirect = '/features';				//set controller link

    protected $FeaturesModel;

    public function __construct(){
        parent::__construct();        $this->data['active'] = 'Product Management';        $this->FeaturesModel = new Featuresmodel();        $this->data['ThisModule'] = $this->FeaturesModel;        $this->data['uploadFolder'] = 'assets/uploads/features';        $this->data['_edit'] = $this->data['admin_link'].$this->_redirect.'/edit';        $this->data['_cancel'] = $this->data['admin_link'].$this->_redirect;        $this->data['_delete'] = $this->data['admin_link'].$this->_redirect.'/delete';        $this->data['_m_cancel'] = $this->data['admin_link'].'/pdcats';
        // for Global List
    ...
    public function l($id=false){        if(!$id){            return redirect()->to($this->data['_m_cancel']);        }
        $this->data['c_id'] = $id;        $this->data['name'] = 'Features';
    ...
    public function edit($c_id=false,$id = NULL){        if(!$c_id){
            return redirect()->to($this->data['_m_cancel']);
        }

        $this->data['_return_url'] = $this->data['_cancel'].'/l/'.$c_id;

                $this->data['c_id'] = $id;        $blank_array = array();
        if($id){
            $this->data['name'] = 'Edit Features';
            $this->data['title'] = $this->data['name'];
            $where['id'] = $id;
            ...
            error_reporting(1);
            $this->data['name'] = 'Create Features';
            $this->data['title'] = $this->data['name'];
            $this->data['thisItems'] = $this->data['ThisModule']->getNew();
            $this->data['thisItems']->image = '';
            $message = 'Data has successfully created';
        }
        $this->data['id'] = $id;        $this->data['templates_page'] = $this->data['ThisModule']->getTemplates('page_');
            ...
            if($input) {
                ...
                //pp($data_lang);
                                if($id == NULL){
                                        ...
                                        $data['created'] = time();
                                        $data['modified'] = time();
                                        $d_action = 'New Feature added.';
                                    }
                                else{
                                        $data['modified'] = time();
                                        $d_action = 'Feature updated.';
                                    }
