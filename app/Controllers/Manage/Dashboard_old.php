...
class Dashboard extends Admincontroller {
    ...
    public function index() {
        ...
        if(count($get_localchat)>0) {
            ...
            foreach($get_localchat as $chat){
                ...
                if($userdata->image != '' && file_exists($_SERVER['DOCUMENT_ROOT'].'/assets/uploads/employees/small/'.$userdata->image)) {
                    $setimage = site_url('/').'assets/uploads/employees/small/'.$userdata->image;
                }
                $addclass= 'in';
                if($chat->sender_id == $emp_id){
                    $addclass= 'out';
                }
                ...
                $htmlshow .= '<li class="'.$addclass.'" data-id="'.$chat->id.'" data-user="'.$chat->sender_id.'">
								<img class="avatar" alt="'.$_SERVER['DOCUMENT_ROOT'].$userdata->image.'" src="'.$setimage.'">
                                <div class="message">
									<span class="arrow"></span>
									<a href="javascript:;" class="name">'.$showusername.'</a>
									<span class="datetime timeago" data-livestamp="'.strtotime($chat->created_at).'"></span>
									<span class="body">'.makeUrltoLink($chat->s_content).'</span>
								</div>
							</li>';
    ...
    public function searchcontent() {
        if ($this->request->getMethod() == "post") {
                ...
                foreach($get_localchat as $chat){
                    ...
                    $htmlshow .= '<li class="'.$addclass.'" data-id="'.$chat->id.'" data-user="'.$chat->sender_id.'" >
                                    ...
                                    <div class="message">
                                        ...
                                        <a href="javascript:;" class="name">'.$showusername.'</a>
                                        <span class="datetime timeago" data-livestamp="'.strtotime($chat->created_at).'"></span>
                                        <span class="body">'.$s_content.'</span>
                                    </div>
                                </li>';
                }
            }
            $jsonrerurn = json_encode(array(
                "status" => 'success',
                "html" => ($htmlshow!='')?$htmlshow:'<li>No data found</li>',
            ));
            $json_pretty = json_encode(json_decode($jsonrerurn), JSON_PRETTY_PRINT);
            echo $json_pretty; 
        }             
        ...
        if(count($get_localchat)>0) {
            ...
            foreach($get_localchat as $chat){
                ...
                $htmlshow .= '<li class="'.$addclass.'" data-id="'.$chat->id.'" data-user="'.$chat->sender_id.'" >
                                ...
                                <div class="message">
									<span class="arrow"></span>
									<a href="javascript:;" class="name"><b>'.$showusername.'</b></a>
									<span class="datetime timeago" data-livestamp="'.strtotime($chat->created_at).'"></span>
									<span class="body">'.makeUrltoLink($chat->s_content).'</span>
								</div>
							</li>';
