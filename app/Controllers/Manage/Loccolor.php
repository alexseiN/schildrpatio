...
use App\Models\backend\Loccolormodel;

class Loccolor extends Admincontroller {
    public $_table_names 	= 'loccolor';		//set table
    public $_subView = 'admin/loccolor/';			//set subview
    public $_mainView = 'admin/_layout_main';		//set mainview
    public $_redirect = '/loccolor';				//set controller link
    protected $PdcatsModel;
    public function __construct(){
        parent::__construct();
        $this->data['active'] = 'Local Store';
        $this->LoccolorModel = new Loccolormodel();
        $this->data['ThisModule'] = $this->LoccolorModel;
        ...
        $main_array_view = array(
                    "view_data"=>array(
                        "view_folder"=>"loccolor",
                        "columns_with_filteroptions"=>$columns_with_filteroptions,
                        "total_count"=>$total_count,
                        "is_filter"=>"no",
                        "default_sort"=>"order",
                        "default_sort_type"=>"ASC",
                        "add_link"=>array("status"=>"yes","link"=>$this->data['admin_link'].'/loccolor/edit'),
                        "order_link"=>"yes"
                    )
                );
        ...
        if(!empty($this->postdata)) {
            ...
            if($input) {
                ...
                if(is_null($id)) {
                    ...
                    $d_action = 'New Color variant for Store product added.';
                }
                else {
                    $d_action = 'Color variant for Store product updated.';
                }
