...
// echo '<pre>';print_r($setproject);die();
...
//echo '<pre>';print_r($thisemploee);die();
...
.standartnotes li {
                ...
                <div class="invoice-header row">
                    <div class="col-sm-6">
                        ...
                        <span>
                            <img height="55px" src="<?=$default_domain?>/assets/uploads/branches/full/<?= $thisbranch->image ?>" />
                        </span>
                        <?php }  else { ?>
                        <span>
                        <img style="height:55px;filter: brightness(10%);" src="<?=$default_domain?>/assets/uploads/sites/<?= $main->logo ?>" />
                        </span>
                        <?php } ?>
                ...
                <div class="invoice-table pagebreak">
                        ...
                        <table class="table table-bordered">
                            <thead class="thead-light">
                                <tr class="s-item">
                                    ...
                                    <th scope="col" class="cvc">Cover Option</th>
                    ...
                    <div class="col-sm-12 text-right " style="margin-top:10px;">
                        <?php if ($total) { ?>
                            <h4>Total: <b><?php if ($thiscurrency->symbol == '$' or $thiscurrency->symbol == '£') { echo $thiscurrency->symbol; } ?>
                                            
                                            <?php if ($percent) { $newTotal = $total + $extratotal + ($total + $extratotal)*$percent/100; } else { $newTotal = $total + $extratotal; } ?>
                                            
                                            <?= money_format('%i', ($newTotal)) ?> 
                                            
                                        <?php if ($thiscurrency->symbol <> '$' and $thiscurrency->symbol <> '£') { echo $thiscurrency->symbol;} ?>
                                                                                                    </b></h4>
                        <? } ?>
                        <hr class="no-print">
                    </div>
                        ...
                        <div class="col-sm-12 text-right" style="">
                            <?php if ($total) { ?>
                                <h4>Total: <b><?php if ($thiscurrency->symbol == '$' or $thiscurrency->symbol == '£') { echo $thiscurrency->symbol; } ?>
                                                ...
                                                <?php if ($thiscurrency->symbol <> '$' and $thiscurrency->symbol <> '£') { echo $thiscurrency->symbol;} ?></b></h4>
                            <? } ?>
                            <hr class="no-print">
                        </div>
                    </div>
        ...
        <script type="text/javascript">
            function printDiv() {
                ...
                // var printContents = document.getElementById('invdet').innerHTML;
                var printContents = $('#invdet>#MainInvoice>.row').get(0).innerHTML;
                // console.log(printContents)
                // var iframe = document.createElement('iframe');
                // iframe.setAttribute('name', 'print-iframe');
                // iframe.setAttribute('id', 'print-iframe');
                // iframe.src="javascript:false";
                // var iframeBody = document.createElement("body");
                // iframe.appendChild(iframeBody);
                // document.body.appendChild(iframe);
                // console.log($("#print-iframe"))
                // var iframe = $("#print-iframe").get(0);
                // console.log(iframe);
                // var printWin = iframe.contentWindow || iframe;
                // console.log(printWin);
                // printWin.document.body.innerHTML = printContents;
