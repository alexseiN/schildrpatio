...
if($objects){
    foreach($objects as $set_data){
		...
		if(!empty($language_data)){
			$image  = !empty($set_data->image)?'assets/uploads/projects/thumbnails/'.$set_data->image:'assets/uploads/no-image.gif';
			foreach ($_thisData['pjcats'] as $cat) { if ($cat->id == $set_data->category) {$material = $cat->title; } }		
?>
<tr>
	<td>
		<div class="d-flex align-items-center">
			...
			<div class="d-flex justify-content-start flex-column">
				<div class="<?=(($set_data->parent_id != 0)?'':'')?> fs-6"><?=$language_data->title?></div>
			</div>
		</div>
	</td>
	<td>
		<span class="fs-6"><span class="badge-light-primary me-1 d-inline-block rounded-0 py-1 px-3"><?=$material?></span></span>
	</td>
    <td>
