...
if($objects){
    foreach($objects as $set_data){
		...
		if(!empty($language_data)){
			$image  = !empty($set_data->image)?'assets/uploads/pjcats/thumbnails/'.$set_data->image:'assets/uploads/no-image.gif';
	...
	<td>
		...
		<div class="d-flex align-items-center <?=(($set_data->parent_id != 0)?'ms-15':'')?>">
			...
			<div class="d-flex justify-content-start flex-column">
				<span class="<?=(($set_data->parent_id != 0)?'text-muted':'text-dark')?> fs-6"><?=$language_data->title?></span>
			</div>
		</div>
