...
if($objects){
    foreach($objects as $set_data){
		...
		if ($set_data->quote == 1) {$quote = 'checked';} else {$quote = ' ';}
		
		$language_data = get_admin_conn_lang($_lang_table_names,$set_data->id,$_thisData['admin_lang']);
		if(!empty($language_data)){
			$image  = !empty($set_data->image)?'assets/uploads/products/small/'.$set_data->image:'assets/uploads/no-image.gif';			
?>
<tr>
	<td>
		<div class="d-flex align-items-center">
			...
			<div class="d-flex justify-content-start flex-column">
				<span class="<?=(($set_data->parent_id != 0)?'':'')?> fs-6 "><?=$language_data->title?></span>
			</div>
		</div>
	</td>
	<td>
		<span class="d-block lead"><span class="badge-light-primary me-1 d-inline-block rounded-0 py-1 px-3"><?=$set_data->type?></span></span>
	</td>
    <td>
		<div class="d-flex justify-content-end flex-shrink-0">
			
			<div class="form-switch switch-sm mt-3">
					<input type="checkbox" class="form-check-input h-20px w-30px me-1" name="sect[]" onclick="do_quote(<?=$set_data->id?>)" <?=$quote?>/>
			</div>
			<div class="form-switch switch-sm mt-3">
